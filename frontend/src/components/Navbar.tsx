...
export default function Navbar() {
  ...
  return (
    <motion.header 
      ...
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'py-3' : 'py-6'}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex justify-between items-center transition-all duration-500 rounded-full px-6 py-3 ${isScrolled ? 'glass' : 'bg-transparent'}`}>
          ...
          <div className="hidden md:flex items-center gap-5">
            <a href="#" className="font-semibold text-sm text-slate-600 hover:text-slate-900 transition-colors">Se connecter</a>
            <button className="group relative bg-slate-900 hover:bg-slate-800 text-white px-6 py-2.5 rounded-full text-sm font-semibold transition-all overflow-hidden flex items-center gap-2">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            <a href="#" className="text-slate-700 font-semibold text-lg py-2">Se connecter</a>
            <button className="w-full bg-slate-900 text-white px-4 py-4 rounded-xl text-center font-bold mt-2 shadow-lg shadow-slate-900/20">
