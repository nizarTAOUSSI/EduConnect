        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-10">
          ...
          <div className="flex flex-col items-center md:items-end gap-6">
            <nav className="flex flex-wrap justify-center gap-8 text-sm font-bold text-slate-400">
              <a href="#fonctionnalites" className="hover:text-white transition-colors">{t('navbar.features')}</a>
              <a href="#" className="hover:text-white transition-colors">{t('footer.contact')}</a>
              <a href="#" className="hover:text-white transition-colors flex items-center gap-1">
                {t('navbar.login')} <ArrowUpRight className="w-3 h-3" />
              </a>
            </nav>
            <p className="text-sm font-medium text-slate-500">
              &copy; {new Date().getFullYear()} EduConnect Inc. {t('footer.rights')}
            </p>
          </div>
