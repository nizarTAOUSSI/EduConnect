...
import { LayoutDashboard, LogOut, ChevronRight, FileSpreadsheet, Clock, Calendar, MessageSquare, Bell, PanelLeftClose, PanelLeftOpen, Languages } from 'lucide-react';
...
export default function StudentSidebar() {
  ...
  const NAV_ITEMS = [
    { name: t('dashboard.sidebar.dashboard'), path: '/dashboard/etudiant', icon: LayoutDashboard },
    { name: t('dashboard.sidebar.my_grades'), path: '/dashboard/etudiant/notes', icon: FileSpreadsheet },
    { name: t('dashboard.sidebar.timetable'), path: '/dashboard/etudiant/timetable', icon: Calendar },
    { name: t('dashboard.sidebar.my_absences'), path: '/dashboard/etudiant/absences', icon: Clock },
    { name: t('dashboard.sidebar.complaints'), path: '/dashboard/etudiant/reclamations', icon: MessageSquare },
    { name: t('dashboard.sidebar.notifications'), path: '/dashboard/etudiant/notifications', icon: Bell, isNotification: true },
  ];
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-white border-r border-slate-200 flex flex-col h-full shrink-0 transition-all duration-300 ease-in-out`}>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto mt-4 custom-scrollbar">
        {NAV_ITEMS.map((item) => {
          const isActive = pathname === item.path || (item.path !== '/dashboard/etudiant' && pathname.startsWith(item.path));
          return (
            <Link
              key={item.path}
              to={item.path}
