...
import { LayoutDashboard, LogOut, ChevronRight, GraduationCap, FileSpreadsheet, Clock, MessageSquare, Calendar, Bell, PanelLeftClose, PanelLeftOpen, Languages } from 'lucide-react';
...
export default function TeacherSidebar() {
  ...
  const NAV_ITEMS = [
    { name: t('dashboard.sidebar.dashboard'), path: '/dashboard/enseignant', icon: LayoutDashboard },
    { name: t('dashboard.sidebar.my_classes'), path: '/dashboard/enseignant/classes', icon: GraduationCap },
    { name: t('dashboard.sidebar.timetable'), path: '/dashboard/enseignant/timetable', icon: Calendar },
    { name: t('dashboard.sidebar.grades_management'), path: '/dashboard/enseignant/notes', icon: FileSpreadsheet },
    { name: t('dashboard.sidebar.absences'), path: '/dashboard/enseignant/absences', icon: Clock },
    { name: t('dashboard.sidebar.complaints'), path: '/dashboard/enseignant/reclamations', icon: MessageSquare },
    { name: t('dashboard.sidebar.notifications'), path: '/dashboard/enseignant/notifications', icon: Bell, isNotification: true },
  ];
    ...
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-white border-r border-slate-200 flex flex-col h-full shrink-0 transition-all duration-300 ease-in-out`}>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto mt-4 custom-scrollbar">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  {item.isNotification && unreadCount > 0 && (
                    <span className={`absolute -top-1 -right-1 w-4 h-4 bg-primary text-white text-[10px] font-black rounded-full flex items-center justify-center ring-2 ring-white ${isCollapsed ? '' : 'animate-bounce'}`}>
        ...
        <button
          ...
          className={`absolute cursor-pointer top-4 right-4 flex items-center gap-3 p-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors text-sm font-medium mb-2`}
        >
