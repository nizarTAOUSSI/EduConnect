  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Calculate real average if no bulletin yet
        ...
        // Sort notes by date (evaluation date) then by creation time
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Dashboard Étudiant</h1>
          <p className="text-slate-500 mt-1 font-medium">
            Bienvenue, {user?.first_name}. Voici un aperçu de tes performances.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Moyenne Générale" value={stats.average.toFixed(2)} icon={TrendingUp} color="bg-blue-600" unit="/20" />
        <StatCard title="Total Absences" value={stats.absences} icon={Clock} color="bg-rose-600" trend={stats.absences > 3 ? "Attention" : "Bon état"} />
        <StatCard title="Mes Matières" value={stats.matieres} icon={BookOpen} color="bg-indigo-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-3xl border border-slate-200/60 p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-xl font-bold text-slate-900">Dernières notes</h2>
          </div>
